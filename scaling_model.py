...
class FinalResNet(nn.Module):
    def __init__(
        ...
        elif delta_type == "fixed":
            self.delta = torch.FloatTensor([1. / self.num_layers])
        else:
            raise ValueError(
                f"Unknown delta_type {delta_type}. Allowed: 'shared', 'multi', 'fixed'."
            )
    ...
    def update_rule(self, x, l):
        ...
        if self.delta_type in ["shared", "fixed"]:
            return torch.abs(self.delta) * h
        elif self.delta_type == "multi":
            return self.delta[l] * h
        else:
            raise ValueError(
                f"Unknown delta_type {self.delta_type}. Allowed: 'shared', 'multi', 'fixed'."
            )
